...
abstract class DataItem {
  ...
  /**
   * Prevents serialization of this data.
   *
   * Subsequent attempts to call serialize() on this data will cause an
   * exception.
   *
   * This is necessary prior to grafting data.
   *
   * @return self
   */
  ...
  public function getItem(string $address): self {
    ...
    foreach ($address_pieces as $child_name) {
      ...
      elseif (is_numeric($child_name)) {
        ...
        if ($ref->isMultiple()) {
          throw new InvalidDataAddressException(sprintf(
            "Unable to get child item '%s' from multiple item with address '%s'.",
            $child_name,
            $ref->getAddress()
          ));
        }
